...
date_vect <- time_span[c(1,length(time_span))]
df <- list()
...
p2 <- ggplot(df_evolution %>% filter(feature1> seuil|feature1<(-seuil)) %>% filter(feature2>seuil|feature2<(-seuil)))  +
  ...
  coord_fixed(ratio = 1)+
  geom_smooth(method='lm', aes(x=feature1, y=feature2))


pdf(file=file.path(dir_plot, paste0(feature2, "_vs_", feature1, "_dynamic8519.pdf")), width = 7, height = 10)
p2
dev.off()
