...
name_ft <- c(feature1, feature2, feature3, feature4)
i<-1
...
p[[1]] <- ggplot(df_f %>% filter(feature1>seuil1)) + 
...
p[[2]] <- ggplot(df_f %>% filter(feature2>seuil2)) + 
...
p[[3]] <- ggplot(df_f %>% filter(feature3>seuil3)) + 
...
p[[4]] <- ggplot(df_f %>% filter(feature4>seuil4)) + 
...
plot(p[[i]])
ggsave(file.path(dir_plot, paste0(name_ft[i],"_evolution.pdf")), dpi = 300)
