...
for (y in years[1:3]){
  sf_list_name <- list.files(path=file.path(dir_data, "vulnerability"), pattern = paste0(y,".+.shp"), full.names = TRUE)
...
p1 <- ggplot(df_f %>% filter(feature1>seuil1)) + 
  ...
  labs(title = paste0(feature1, " % per hydrobasin (filtered above ", seuil, ")"), x="year", y="")+ 
  theme_bw()+
...
p2 <- ggplot(df_f %>% filter(feature2>seuil2)) + 
  ...
  labs(title = paste0(feature2, " % per hydrobasin (filtered above ", seuil, ")"), x="year", y= "")+ 
  theme_bw()+
  theme(axis.text.x = element_text(size = 12), axis.title.x = element_text(size = 10, vjust = 1.5, hjust=1),
        axis.text.y = element_text(size = 12), axis.title.y = element_text(size = 10, angle = 0, vjust=1.05),
        plot.title = element_text(size = 10, face = "bold", hjust=0.5))
...
pdf(file=file.path(dir_plot, "boxplot", "evolution.pdf"), width = 7, height = 7)
p1 + p2 + p3 + p4 
dev.off()
