...
tictoc::tic()
  ...
  foreach(i=hybas_id, .packages=c('sf')) %dopar%  { #
    ...
    st_write(crop_shp, file.path(dir_data, "processed",paste0("riossimple_poly",i,".shp")), delete_layer = TRUE)
...
polynetwork <- st_read(file.path(dir_data, "water_surface", "rios_poly", "output_hidro_poly.shp"))
  ...
  foreach(i=hybas_id, .packages=c('sf')) %dopar%  { #
    ...
    st_write(poly_shp, file.path(dir_data, "processed", paste0("riospoly_poly",i,".shp")), delete_layer = TRUE)
  ...
  foreach(i=hybas_id, .packages=c('sf')) %dopar%  { #
    ...
    st_write(lago_shp, file.path(dir_data, "processed",paste0("lago_poly",i,".shp")), delete_layer = TRUE)
  ...
  foreach(i=hybas_id, .packages=c('sf')) %dopar%  { #
    ...
    st_write(nascente_shp, file.path(dir_data, "processed",paste0("source_poly",i,".shp")), delete_layer = TRUE)
